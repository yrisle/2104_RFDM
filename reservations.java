...
public class reservations extends javax.swing.JFrame {
    ...
    public reservations(String selectedItems, String clientName) {
        initComponents();
        txtCname.setText(clientName); // Set the client's name

        // Parse the selected items string and populate the table
        ...
        for (String item : items) {
            if (!item.trim().isEmpty()) {
                // Assuming the format is `"ID, Type, Name, Price, Remarks, Category"`
                String[] details = item.split(", ");
                int id = Integer.parseInt(details[0].trim()); // Extract ID
                String type = details[1].trim(); // Extract Type
                String name = details[2].trim(); // Extract Name
                int price = Integer.parseInt(details[3].trim()); // Extract Price
                String remarks = details[4].trim(); // Extract Remarks
                String category = details[5].trim(); // Extract Category
                
                // Add the selected item to the table model
    ...
    // Calculate the total payment based on the items in the table
    double totalPayment = 0.0;
    DefaultTableModel model = (DefaultTableModel) jTable1.getModel();

    // Iterate through the table rows and sum the prices
    for (int i = 0; i < model.getRowCount(); i++) {
        totalPayment += ((Number) model.getValueAt(i, 3)).doubleValue();
 // Assuming price is in the 4th column (index 3)
    }

    // Get the client name from the text field
    String clientName = txtCname.getText();

    // Pass the total payment and client name to the payment_page
    payment_page pay = new payment_page(totalPayment, clientName);
    pay.setVisible(true);
    this.dispose(); // Close the reservations page
